...
function openTokensModal(uid, userName, currentTokens) {
  openModal(`Tokens - ${userName}`, `
    <div class="tokens-modal-content">
      <div class="tokens-current">
        <span class="tokens-label">Tokens actuales</span>
        <span class="tokens-value">${currentTokens}</span>
      </div>
      
      <div class="tokens-change-section">
        <span class="tokens-change-label">Cambio a aplicar:</span>
        <div class="tokens-controls">
          <button type="button" class="tokens-btn tokens-btn-minus" onclick="decrementTokens()">−</button>
          <input type="number" id="tokens-amount" class="tokens-input" value="0" 
                 onchange="updateTokensPreview(${currentTokens})" 
                 oninput="updateTokensPreview(${currentTokens})">
          <button type="button" class="tokens-btn tokens-btn-plus" onclick="incrementTokens()">+</button>
        </div>
      </div>
      
      <div class="tokens-preview" id="tokens-preview">
        <span class="tokens-preview-label">Resultado:</span>
        <span class="tokens-preview-value" id="tokens-result">${currentTokens}</span>
      </div>
    </div>
  `, `
    <button class="btn btn-secondary" onclick="closeModal()">Cancelar</button>
  ...
  // Guardar el valor actual para calcular preview
  window.currentUserTokens = currentTokens;
}

// Funciones para los botones + y -
function incrementTokens() {
  const input = document.getElementById('tokens-amount');
  let value = parseInt(input.value) || 0;
  input.value = value + 1;
  updateTokensPreview(window.currentUserTokens);
}
...
function updateTokensPreview(currentTokens) {
  const input = document.getElementById('tokens-amount');
  const resultEl = document.getElementById('tokens-result');
  const previewEl = document.getElementById('tokens-preview');

  const change = parseInt(input.value) || 0;
  const newTotal = Math.max(0, currentTokens + change);

  resultEl.textContent = newTotal;

  // Colorear según el cambio
  previewEl.classList.remove('positive', 'negative', 'neutral');
  if (change > 0) {
    previewEl.classList.add('positive');
  } else if (change < 0) {
    previewEl.classList.add('negative');
  } else {
    previewEl.classList.add('neutral');
  }
}
